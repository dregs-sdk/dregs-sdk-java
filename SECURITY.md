...
Verify every webhook before acting on it, against the **raw request body**. `Webhooks.verify` does the
HMAC comparison in constant time and rejects stale payloads as replays. Verifying a re-serialized
object instead of the received bytes will not match, and working around that by skipping verification
would leave the endpoint open to anyone who learns its URL.

In practice this means binding the body as `byte[]` or `String` rather than as a mapped type, and
reading a servlet's input stream exactly once.
...
Both the dependency versions (`gradle.lockfile`) and their checksums
(`gradle/verification-metadata.xml`) are committed, and CI fails on either being stale. Gson is the
only runtime dependency. Releases are signed with the project's GPG key and published to Maven
Central from a workflow behind an environment gate.
